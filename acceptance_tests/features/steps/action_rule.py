...
@step('an export file action rule has been created for packcode "{packcode}" with the classifier '
      'from action rule {action_rule_id}')
def create_export_file_action_rule_with_classifiers(context, packcode, action_rule_id):
    ...
    matches = [record for record in context.action_rules if
               str(record.get("packCode")).strip('"') == cleaned_pack_code
               and str(record.get("actionRuleId")).strip("'") == cleaned_action_rule_id]
